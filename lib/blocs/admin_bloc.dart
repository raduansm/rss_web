...
class AdminBloc extends ChangeNotifier {
  ...
  Future getAdsData() async {
    await firestore.collection('admin').doc('ads').get().then((value) async {
      if (value.exists) {
        _bannerAd = value['banner_ad'];
        _interstitialAd = value['interstitial_ad'];
      } else {
        await firestore.collection('admin').doc('ads').set({
          'banner_ad': false,
  ...
  Future controlBannerAd(bool value, context) async {
    await firestore.collection('admin').doc('ads').update({'banner_ad': value}).then((_) {
      _bannerAd = value;
      if (value == true) {
  ...
  Future controlInterstitialAd(bool value, context) async {
    await firestore.collection('admin').doc('ads').update({'interstitial_ad': value}).then((_) {
  ...
  Future getAdminPass() async {
    firestore.collection('admin').doc('user type').get().then((DocumentSnapshot snap) {
  ...
  Future increaseCount(String documentName) async {
    await getTotalDocuments(documentName).then((int documentCount) async {
      await firestore.collection('item_count').doc(documentName).update({'count': documentCount + 1});
    });
  }
  ...
  Future getRSSFEEDS() async {
    await firestore.collection('rss_feeds').limit(1).get().then((value) async {
      if (value.size != 0) {
        QuerySnapshot snap = await firestore.collection('rss_feeds').get();
        List d = snap.docs;
        _rssFeeds.clear();
        d.forEach((element) {
          _rssFeeds.add(element['name']);
        });
      } else {
        _rssFeeds.clear();
      }
  ...
  Future removefromFeaturedList(context, String? timestamp) async {
    ...
    await getFeaturedList().then((featuredList) async {
      if (featuredList.contains(timestamp)) {
        await ref.update({
          'contents': FieldValue.arrayRemove([timestamp])
        });
  ...
  Future saveNewAdminPassword(String newPassword) async {
    await firestore.collection('admin').doc('user type').update({'admin password': newPassword}).then((value) => getAdminPass());
    notifyListeners();
  }
}
