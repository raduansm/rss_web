...
class _CategoriesState extends State<Categories> {
                  ...
                  TextButton(
                    ...
                    onPressed: () async {
                      if (ab.userType == 'tester') {
                        ...
                        await ab.deleteContent(timestamp1, collectionName).then((value) => ab.getCategories()).then((value) => ab.decreaseCount('categories_count')).then((value) => openToast(context, 'Deleted Successfully'));
                        refreshData();
                        Navigator.pop(context);
                      }
                    },
  ...
  Widget dataList(RSSFEED d) {
    return Container(
      ...
      decoration: BoxDecoration(color: Colors.grey[200], borderRadius: BorderRadius.circular(10), image: DecorationImage(image: CachedNetworkImageProvider(d.thumbnailUrl!), fit: BoxFit.cover)),
      child: Row(
  ...
  Future addCategory() async {
    ...
    await ref.set({'name': nameCtrl.text, 'thumbnail': thumbnailCtrl.text, 'timestamp': timestamp});
  }
  ...
  handleAddCategory() async {
    ...
    if (formKey.currentState!.validate()) {
      ...
      if (ab.userType == 'tester') {
        ...
        await getTimestamp().then((value) => addCategory()).then((value) => context.read<AdminBloc>().increaseCount('categories_count')).then((value) => openToast(context, 'Added Successfully')).then((value) => ab.getCategories());
        refreshData();
        Navigator.pop(context);
      }
    }
  ...
  Future _handleUpdateCategory(String categoryTimestamp) async {
    ...
    if (formKey1.currentState!.validate()) {
      ...
      if (ab.userType == 'tester') {
        ...
        await _updateCategory(categoryTimestamp).then((value) => openToast(context, 'Updated Successfully')).then((value) => ab.getCategories());
        refreshData();
        Navigator.pop(context);
      }
    }
